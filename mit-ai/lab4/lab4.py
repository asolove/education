...
def forward_checking_prop_singleton(state, verbose=False):
    ...
    singletons = filter(lambda v: len(v.get_domain()) == 1, state.get_all_variables())
    visited = []
    while singletons:
        var1 = singletons.pop()
        val1 = var1.get_domain()[0]
        visited.append(var1)
        ...
        for c in state.get_constraints_by_name(var1.get_name()):
            ...
            new_domain_size = len(var2.get_domain())
            if new_domain_size == 0:
                return False
            elif new_domain_size == 1 and var2 not in visited:
                singletons.append(var2)
